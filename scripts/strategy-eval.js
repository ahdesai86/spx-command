 ...
 *
 * Usage: node scripts/strategy-eval.js [--url ...] [--user U --pass P] [--days 60]
...
(async()=>{
  const trades=((await get("/db/trades?limit=1000")).trades||[]).filter(t=>t.pnl!=null);
  const sigs=((await get("/db/signals?limit=5000")).signals||[]);
  const days=[...new Set(trades.map(t=>t.date))].sort().slice(-DAYS);
  const T=trades.filter(t=>days.includes(t.date));

  // ── 1. Expectancy by regime ────────────────────────────────────────────────
  console.log("\n════ 1. EXPECTANCY  ("+days[0]+" … "+days[days.length-1]+", "+T.length+" trades) ════");
  ...
  // Per 1% of exit slippage: cost = 1% × exit premium × contracts × 100, summed. A sensitivity,
  // not a total — recorded P&L already reflects real fills post-v11.33.
  ...
  for(const s of sigs){ if(s.fired||!days.includes(s.date))continue; if(s.direction!=="LONG"&&s.direction!=="SHORT")continue;
    const c=cat(s.blocked_reason); if(!c)continue; const o=fwd(s.direction,s.spy_price,s.date,s.timestamp);
